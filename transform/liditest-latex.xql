    ...
    $input !         (
            ...
            return
                    ...
                    case element(figure) return
                        latex:figure($config, ., ("tei-figure", css:map-rend-to-class(.)), ., @figDesc)
                    ...
                    case element(author) return
                        if ($parameters?mode='commentary') then
                            ...
                            if (parent::bibl) then
                                latex:inline($config, ., ("tei-author2", css:map-rend-to-class(.)), id (substring-after(@ref, '#'), doc("/db/apps/LiDi/data/bibl.xml")))
                            else
                    ...
                    case element(ptr) return
                        latex:link($config, ., ("tei-ptr1", css:map-rend-to-class(.)), "🔗", @target, map {})
                    ...
                    case element(pb) return
                        (
                            ...
                            $config?apply($config, ./node()),
                            latex:break($config, ., css:get-rendition(., ("tei-pb2", css:map-rend-to-class(.))), ., 'page', if(@n) then concat(@n,' ') else '')
                        )
                    ...
                    case element(bibl) return
                        if (ancestor::body) then
                            latex:alternate($config, ., ("tei-bibl", css:map-rend-to-class(.)), ., ., id (substring-after(@source, '#'), doc("/db/apps/LiDi/data/bibl.xml")))
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(title) return
                        if (parent::bibl) then
                            latex:inline($config, ., ("tei-title1", css:map-rend-to-class(.)), .)
                        else
                            if (parent::titleStmt/parent::fileDesc) then
                                ...
                                if ($parameters?header='short') then
                                    latex:heading($config, ., ("tei-title6", css:map-rend-to-class(.)), ., 5)
                                else
                                            ...
                                            if (@level='s' or @level='j') then
                                                ...
                                                if (@level='u' or @level='a') then
                                                    (
                                                        ...
                                                        if (following-sibling::* and     (    ancestor::biblFull)) then
                                                            latex:text($config, ., ("tei-title13", css:map-rend-to-class(.)), '. ')
                                                        else
                                                            ()
                                                    )
                                                    ...
                                                    (
                                                        latex:inline($config, ., ("tei-title4", css:map-rend-to-class(.)), id (substring-after(@ref, '#'), doc("/db/apps/LiDi/data/bibl.xml"))),
                                                        latex:link($config, ., ("tei-title5", css:map-rend-to-class(.)), ., @sameAs, map {})
                                                    )

                    case element(date) return
                        if ($parameters?mode='commentary') then
                            latex:block($config, ., ("tei-date1", css:map-rend-to-class(.)), .)
                        else
                        ...
                        if ($parameters?mode='commentary') then
                            ...
                            if ($parameters?header='short') then
                                (
                                    ...
                                    (: Output abstract :)
                                    latex:block($config, ., ("tei-fileDesc5", "sample-description", css:map-rend-to-class(.)), //note/ab)
                                )
                    ...
                    case element(quote) return
                        latex:alternate($config, ., ("tei-quote", css:map-rend-to-class(.)), ., ., id (substring-after(@source, '#'), doc("/db/apps/LiDi/data/bibl.xml")))
                    ...
                    case element(seg) return
                        if (@xml:lang and @source) then
                            ...
                            return
                                                        latex:figure(map:merge(($config, map:entry("template", true()))), ., ("tei-seg1", css:map-rend-to-class(.)), $content, ())
                        else
                            if (@xml:lang) then
                                latex:figure($config, ., ("tei-seg2", css:map-rend-to-class(.)), ., ())
                            else
                                if (@source) then
                                    latex:link($config, ., ("tei-seg3", css:map-rend-to-class(.)), ., @source, map {"target": "_blank"})
                                else
                                    latex:inline($config, ., ("tei-seg4", css:map-rend-to-class(.)), .)
                    ...
                    case element(cb) return
                        latex:omit($config, ., ("tei-cb", css:map-rend-to-class(.)), .)
                    ...
                    case element(person) return
                        if (@sameAs) then
                            latex:link($config, ., ("tei-person1", css:map-rend-to-class(.)), persName, @sameAs, map {"target": "_blank"})
                        else
                            latex:inline($config, ., ("tei-person2", css:map-rend-to-class(.)), persName)
                    case element(place) return
                        if (@sameAs) then
                            latex:link($config, ., ("tei-place1", css:map-rend-to-class(.)), ., @sameAs, map {"target": "_blank"})
                        else
                            latex:inline($config, ., ("tei-place2", css:map-rend-to-class(.)), placeName)
                    ...
                    case element(sourceDesc) return
                        if ($parameters?mode='commentary') then
                            latex:block($config, ., ("tei-sourceDesc", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(physDesc) return
                        if ($parameters?mode='commentary') then
                            latex:block($config, ., ("tei-physDesc", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(figDesc) return
                        latex:alternate($config, ., ("tei-figDesc", css:map-rend-to-class(.)), ., "🖼️", .)
                    case element(term) return
                        ...
                        return
                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-term", css:map-rend-to-class(.)), $content)
                    case element(classDecl) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-classDecl", css:map-rend-to-class(.)), $content)
                        else
                            $config?apply($config, ./node())
                    case element() return
