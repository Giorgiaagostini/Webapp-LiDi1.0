    ...
    $input !         (
            ...
            return
                    ...
                    case element(quote) return
                        html:alternate($config, ., ("tei-quote1", css:map-rend-to-class(.)), ., ., id (substring-after(@source, '#'), doc("/db/apps/LiDi/data/bibl.xml")), map {})                        => model:map($node, $trackIds)
                    ...
                    case element(seg) return
                        if (@source) then
                            html:link($config, ., ("tei-seg1", css:map-rend-to-class(.)), ., @source, "_blank", map {})                            => model:map($node, $trackIds)
                        else
                            html:inline($config, ., ("tei-seg2", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
