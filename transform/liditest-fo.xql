...
module namespace model="http://www.tei-c.org/pm/models/liditest/fo";
            ...
            return
                    ...
                    case element(licence) return
                        if ($parameters?mode='commentary') then
                            fo:link($config, ., ("tei-licence1", css:map-rend-to-class(.)), ., @target, map {})
                        else
                    ...
                    case element(figure) return
                        fo:figure($config, ., ("tei-figure", css:map-rend-to-class(.)), ., @figDesc)
                    ...
                    case element(author) return
                        if ($parameters?mode='commentary') then
                            ...
                            if (parent::bibl) then
                                fo:inline($config, ., ("tei-author2", css:map-rend-to-class(.)), id (substring-after(@ref, '#'), doc("/db/apps/LiDi/data/bibl.xml")))
                            else
                                if (ancestor::teiHeader) then
                                    fo:block($config, ., ("tei-author3", css:map-rend-to-class(.)), .)
                                else
                    ...
                    case element(ptr) return
                        fo:link($config, ., ("tei-ptr1", css:map-rend-to-class(.)), "🔗", @target, map {})
                    ...
                    case element(pb) return
                        (
                            ...
                            $config?apply($config, ./node()),
                            fo:break($config, ., css:get-rendition(., ("tei-pb2", css:map-rend-to-class(.))), ., 'page', if(@n) then concat(@n,' ') else '')
                        )

                    case element(epigraph) return
                        fo:block($config, ., ("tei-epigraph", css:map-rend-to-class(.)), .)
                    case element(lb) return
                        fo:break($config, ., css:get-rendition(., ("tei-lb1", css:map-rend-to-class(.))), ., 'line', @n)
                    ...
                    case element(publicationStmt) return
                        if ($parameters?mode='commentary') then
                            fo:block($config, ., ("tei-publicationStmt1", css:map-rend-to-class(.)), .)
                        else
                    ...
                    case element(note) return
                        if (@place="margin-right" or @place="margin-left") then
                            fo:note($config, ., ("tei-note", css:map-rend-to-class(.)), ., "margin", ())
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(ref) return
                        if ($parameters?mode='commentary') then
                            fo:link($config, ., ("tei-ref1", css:map-rend-to-class(.)), @target, @target, map {})
                        else
                            if (@target) then
                    ...
                    case element(bibl) return
                        if (ancestor::body) then
                            fo:alternate($config, ., ("tei-bibl", css:map-rend-to-class(.)), ., ., id (substring-after(@source, '#'), doc("/db/apps/LiDi/data/bibl.xml")))
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(title) return
                        if (parent::bibl) then
                            fo:inline($config, ., ("tei-title1", css:map-rend-to-class(.)), .)
                        else
                            if (parent::titleStmt/parent::fileDesc) then
                                ...
                                if ($parameters?header='short') then
                                    fo:heading($config, ., ("tei-title6", css:map-rend-to-class(.)), ., 5)
                                else
                                            ...
                                            if (@level='s' or @level='j') then
                                                ...
                                                if (@level='u' or @level='a') then
                                                    (
                                                        ...
                                                        if (following-sibling::* and     (    ancestor::biblFull)) then
                                                            fo:text($config, ., ("tei-title13", css:map-rend-to-class(.)), '. ')
                                                        else
                                                            ()
                                                    )
                                                    ...
                                                    (
                                                        fo:inline($config, ., ("tei-title4", css:map-rend-to-class(.)), id (substring-after(@ref, '#'), doc("/db/apps/LiDi/data/bibl.xml"))),
                                                        fo:link($config, ., ("tei-title5", css:map-rend-to-class(.)), ., @sameAs, map {})
                                                    )

                    case element(date) return
                        if ($parameters?mode='commentary') then
                            fo:block($config, ., ("tei-date1", css:map-rend-to-class(.)), .)
                        else
                    ...
                    case element(titleStmt) return
                        if ($parameters?mode='commentary') then
                            ...
                            if ($parameters?mode='title') then
                                fo:heading($config, ., ("tei-titleStmt4", css:map-rend-to-class(.)), title[not(@type)], 5)
                            else
                        ...
                        if ($parameters?mode='commentary') then
                            ...
                            if ($parameters?header='short') then
                                (
                                    ...
                                    fo:block($config, ., ("tei-fileDesc3", "header-short", css:map-rend-to-class(.)), /seriesStmt/title | (/seriesStmt/biblScope[position()=1]/@unit | /seriesStmt/biblScope[position()=1]/@n)),
                                    fo:block($config, ., ("tei-fileDesc4", "header-short", css:map-rend-to-class(.)), //msIdentifier/repository),
                                    (: Output abstract :)
                                    fo:block($config, ., ("tei-fileDesc5", "sample-description", css:map-rend-to-class(.)), //note/ab)
                                )
                    ...
                    case element(fw) return
                        if (@type="running-head") then
                            fo:block($config, ., ("tei-fw1", css:map-rend-to-class(.)), .)
                        else
                            if (@type="page-number") then
                                fo:block($config, ., ("tei-fw2", css:map-rend-to-class(.)), .)
                            else
                                if (ancestor::p or ancestor::ab) then
                                    fo:inline($config, ., ("tei-fw3", css:map-rend-to-class(.)), .)
                                else
                                    fo:block($config, ., ("tei-fw4", css:map-rend-to-class(.)), .)
                    case element(encodingDesc) return
                        if ($parameters?header='short') then
                            fo:omit($config, ., ("tei-encodingDesc1", css:map-rend-to-class(.)), .)
                        else
                            if ($parameters?mode='commentary') then
                                let $params := 
                                    map {
                                ...
                                return
                                                                fo:block(map:merge(($config, map:entry("template", true()))), ., ("tei-encodingDesc2", css:map-rend-to-class(.)), $content)
                            else
                                fo:inline($config, ., ("tei-encodingDesc3", css:map-rend-to-class(.)), .)
                    case element(quote) return
                        fo:alternate($config, ., ("tei-quote", css:map-rend-to-class(.)), ., ., id (substring-after(@source, '#'), doc("/db/apps/LiDi/data/bibl.xml")))
                    ...
                    case element(seg) return
                        if (@xml:lang and @source) then
                            ...
                            return
                                                        fo:figure(map:merge(($config, map:entry("template", true()))), ., ("tei-seg1", css:map-rend-to-class(.)), $content, ())
                        else
                            if (@xml:lang) then
                                fo:figure($config, ., ("tei-seg2", css:map-rend-to-class(.)), ., ())
                            else
                                if (@source) then
                                    fo:link($config, ., ("tei-seg3", css:map-rend-to-class(.)), ., @source, map {"target": "_blank"})
                                else
                                    fo:inline($config, ., ("tei-seg4", css:map-rend-to-class(.)), .)
                    ...
                    case element(cb) return
                        fo:omit($config, ., ("tei-cb", css:map-rend-to-class(.)), .)
                    ...
                    case element(placeName) return
                        if (ancestor::body) then
                            fo:alternate($config, ., ("tei-placeName", css:map-rend-to-class(.)), ., ., id (substring-after(@ref, '#'), doc("/db/apps/LiDi/data/place.xml")))
                        else
                            $config?apply($config, ./node())
                    case element(person) return
                        if (@sameAs) then
                            fo:link($config, ., ("tei-person1", css:map-rend-to-class(.)), persName, @sameAs, map {"target": "_blank"})
                        else
                            fo:inline($config, ., ("tei-person2", css:map-rend-to-class(.)), persName)
                    case element(place) return
                        if (@sameAs) then
                            fo:link($config, ., ("tei-place1", css:map-rend-to-class(.)), ., @sameAs, map {"target": "_blank"})
                        else
                            fo:inline($config, ., ("tei-place2", css:map-rend-to-class(.)), placeName)
                    ...
                    case element(citedRange) return
                        fo:inline($config, ., ("tei-citedRange", css:map-rend-to-class(.)), .)
                    case element(sponsor) return
                        fo:omit($config, ., ("tei-sponsor", css:map-rend-to-class(.)), .)
                    case element(editionStmt) return
                        if ($parameters?mode='commentary') then
                            fo:block($config, ., ("tei-editionStmt", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    case element(respStmt) return
                        fo:omit($config, ., ("tei-respStmt", css:map-rend-to-class(.)), .)
                    case element(principal) return
                        if ($parameters?mode='commentary') then
                            fo:block($config, ., ("tei-principal", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    case element(funder) return
                        fo:omit($config, ., ("tei-funder", css:map-rend-to-class(.)), .)
                    case element(sourceDesc) return
                        if ($parameters?mode='commentary') then
                            fo:block($config, ., ("tei-sourceDesc", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    case element(msDesc) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        fo:block(map:merge(($config, map:entry("template", true()))), ., ("tei-msDesc1", css:map-rend-to-class(.)), $content)
                        else
                            fo:inline($config, ., ("tei-msDesc2", css:map-rend-to-class(.)), .)
                    case element(physDesc) return
                        if ($parameters?mode='commentary') then
                            fo:block($config, ., ("tei-physDesc", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    case element(seriesStmt) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        fo:block(map:merge(($config, map:entry("template", true()))), ., ("tei-seriesStmt", css:map-rend-to-class(.)), $content)
                        else
                            $config?apply($config, ./node())
                    case element(editorialDecl) return
                        if ($parameters?mode='commentary') then
                            fo:block($config, ., ("tei-editorialDecl1", css:map-rend-to-class(.)), .)
                        else
                            if ($parameters?header='short') then
                                fo:omit($config, ., ("tei-editorialDecl2", css:map-rend-to-class(.)), .)
                            else
                                $config?apply($config, ./node())
                    case element(availability) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        fo:block(map:merge(($config, map:entry("template", true()))), ., ("tei-availability", css:map-rend-to-class(.)), $content)
                        else
                            $config?apply($config, ./node())
                    case element(figDesc) return
                        fo:alternate($config, ., ("tei-figDesc", css:map-rend-to-class(.)), ., "🖼️", .)
                    case element(term) return
                        ...
                        return
                                                fo:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-term", css:map-rend-to-class(.)), $content)
                    case element(classDecl) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        fo:block(map:merge(($config, map:entry("template", true()))), ., ("tei-classDecl", css:map-rend-to-class(.)), $content)
                        else
                            $config?apply($config, ./node())
                    case element() return
