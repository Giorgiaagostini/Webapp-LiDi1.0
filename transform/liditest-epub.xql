...
module namespace model="http://www.tei-c.org/pm/models/liditest/epub";
            ...
            return
                        ...
                        if ($parameters?mode='commentary') then
                            ...
                            if (parent::bibl) then
                                ...
                                if (ancestor::teiHeader) then
                                    epub:block($config, ., ("tei-author3", css:map-rend-to-class(.)), .)
                                else
                    ...
                    case element(pb) return
                        (
                            ...
                            epub:break($config, ., css:get-rendition(., ("tei-pb2", css:map-rend-to-class(.))), ., 'page', if(@n) then concat(@n,' ') else '')
                        )

                    case element(epigraph) return
                        epub:block($config, ., ("tei-epigraph", css:map-rend-to-class(.)), .)
                    case element(lb) return
                        epub:break($config, ., css:get-rendition(., ("tei-lb1", css:map-rend-to-class(.))), ., 'line', @n)
                    ...
                    case element(publicationStmt) return
                        if ($parameters?mode='commentary') then
                            epub:block($config, ., ("tei-publicationStmt1", css:map-rend-to-class(.)), .)
                        else
                    ...
                    case element(note) return
                        if (@place="margin-right" or @place="margin-left") then
                            epub:note($config, ., ("tei-note", css:map-rend-to-class(.)), ., "margin", ())
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(bibl) return
                        if (ancestor::body) then
                            epub:alternate($config, ., ("tei-bibl", css:map-rend-to-class(.)), ., ., id (substring-after(@source, '#'), doc("/db/apps/LiDi/data/bibl.xml")))
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(date) return
                        if ($parameters?mode='commentary') then
                            epub:block($config, ., ("tei-date1", css:map-rend-to-class(.)), .)
                        else
                            if (@when) then
                                epub:alternate($config, ., ("tei-date2", css:map-rend-to-class(.)), ., ., @when)
                            else
                            ...
                            if ($parameters?mode='title') then
                                ...
                                if ($parameters?header='short') then
                                    (
                                        ...
                                        epub:block($config, ., ("tei-titleStmt6", css:map-rend-to-class(.)), subsequence(title, 2)),
                                        epub:block($config, ., ("tei-titleStmt7", css:map-rend-to-class(.)), author)
                                    )
                        ...
                        if ($parameters?mode='commentary') then
                            ...
                            if ($parameters?header='short') then
                                (
                                    ...
                                    epub:block($config, ., ("tei-fileDesc3", "header-short", css:map-rend-to-class(.)), /seriesStmt/title | (/seriesStmt/biblScope[position()=1]/@unit | /seriesStmt/biblScope[position()=1]/@n)),
                                    epub:block($config, ., ("tei-fileDesc4", "header-short", css:map-rend-to-class(.)), //msIdentifier/repository),
                                    (: Output abstract :)
                                    epub:block($config, ., ("tei-fileDesc5", "sample-description", css:map-rend-to-class(.)), //note/ab)
                                )
                    ...
                    case element(encodingDesc) return
                        if ($parameters?header='short') then
                            ...
                            if ($parameters?mode='commentary') then
                                ...
                                return
                                                                epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-encodingDesc2", css:map-rend-to-class(.)), $content)
                            else
                                html:inline($config, ., ("tei-encodingDesc3", css:map-rend-to-class(.)), .)
                    case element(quote) return
                        epub:alternate($config, ., ("tei-quote", css:map-rend-to-class(.)), ., ., id (substring-after(@source, '#'), doc("/db/apps/LiDi/data/bibl.xml")))
                    ...
                    case element(persName) return
                        if (ancestor::listPerson) then
                            ...
                            if (ancestor::body) then
                                epub:alternate($config, ., ("tei-persName2", css:map-rend-to-class(.)), ., ., id (substring-after(@ref, '#'), doc("/db/apps/LiDi/data/people.xml")))
                            else
                                $config?apply($config, ./node())
                    case element(placeName) return
                        if (ancestor::body) then
                            epub:alternate($config, ., ("tei-placeName", css:map-rend-to-class(.)), ., ., id (substring-after(@ref, '#'), doc("/db/apps/LiDi/data/place.xml")))
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(editionStmt) return
                        if ($parameters?mode='commentary') then
                            epub:block($config, ., ("tei-editionStmt", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(principal) return
                        if ($parameters?mode='commentary') then
                            epub:block($config, ., ("tei-principal", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(sourceDesc) return
                        if ($parameters?mode='commentary') then
                            epub:block($config, ., ("tei-sourceDesc", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    case element(msDesc) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-msDesc1", css:map-rend-to-class(.)), $content)
                        else
                            html:inline($config, ., ("tei-msDesc2", css:map-rend-to-class(.)), .)
                    case element(physDesc) return
                        if ($parameters?mode='commentary') then
                            epub:block($config, ., ("tei-physDesc", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    case element(seriesStmt) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-seriesStmt", css:map-rend-to-class(.)), $content)
                        else
                            $config?apply($config, ./node())
                    case element(editorialDecl) return
                        if ($parameters?mode='commentary') then
                            epub:block($config, ., ("tei-editorialDecl1", css:map-rend-to-class(.)), .)
                        else
                            if ($parameters?header='short') then
                    ...
                    case element(availability) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-availability", css:map-rend-to-class(.)), $content)
                        else
                            $config?apply($config, ./node())
                    case element(figDesc) return
                        epub:alternate($config, ., ("tei-figDesc", css:map-rend-to-class(.)), ., "🖼️", .)
                    ...
                    case element(classDecl) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-classDecl", css:map-rend-to-class(.)), $content)
                        else
                            $config?apply($config, ./node())
