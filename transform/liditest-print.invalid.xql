    ...
    $input !         (
            ...
            return
                    ...
                    case element(author) return
                        if ($parameters?mode='commentary') then
                            ...
                            if (ancestor::teiHeader) then
                                ...
                                html:block($config, ., ("tei-author2", css:map-rend-to-class(.)), id (substring-after(@ref, '#'), doc("/db/apps/LiDi/data/bibl.xml")))
                    ...
                    case element(ptr) return
                        if (@target) then
                            html:inline($config, ., ("tei-ptr1", css:map-rend-to-class(.)), .)
                        else
                            if (parent::notatedMusic) then
                                ...
                                return
                                                                html:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-ptr2", css:map-rend-to-class(.)), $content)
                            else
                                $config?apply($config, ./node())
                    ...
                    case element(bibl) return
                        if (ancestor::body) then
                            printcss:alternate($config, ., ("tei-bibl", css:map-rend-to-class(.)), ., ., id (substring-after(@ref, '#'), doc("/db/apps/LiDi/data/bibl.xml")))
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(title) return
                        if (parent::titleStmt/parent::fileDesc) then
                            ...
                            if ($parameters?header='short') then
                                html:heading($config, ., ("tei-title5", css:map-rend-to-class(.)), ., 5)
                            else
                                    ...
                                    if (@level='m' or not(@level)) then
                                        ...
                                        if (@level='s' or @level='j') then
                                            ...
                                            if (@level='u' or @level='a') then
                                                ...
                                                (
                                                    html:inline($config, ., ("tei-title3", css:map-rend-to-class(.)), id (substring-after(@ref, '#'), doc("/db/apps/LiDi/data/bibl.xml"))),
                                                    html:link($config, ., ("tei-title4", css:map-rend-to-class(.)), ., @sameAs, (), map {})
                                                )
                        ...
                        if ($parameters?mode='commentary') then
                            ...
                            if ($parameters?header='short') then
                                (
                                    ...
                                    (: Output abstract :)
                                    html:block($config, ., ("tei-fileDesc5", "sample-description", css:map-rend-to-class(.)), //msContents/ab)
                                )
                    ...
                    case element(quote) return
                        printcss:alternate($config, ., ("tei-quote1", css:map-rend-to-class(.)), ., ., id (substring-after(@ref, '#'), doc("/db/apps/LiDi/data/bibl.xml")))
                    ...
                    case element(seg) return
                        (
                            if (@source) then
                                html:link($config, ., ("tei-seg", css:map-rend-to-class(.)), ., @source, (), map {})
                            else
                                ()
                        )
                    ...
                    case element(person) return
                        (
                            html:link($config, ., ("tei-person", css:map-rend-to-class(.)), persName, @sameAs, (), map {})
                        )

                    case element(place) return
                        (
                            html:block($config, ., ("tei-place1", css:map-rend-to-class(.)), placeName),
                            html:link($config, ., ("tei-place2", css:map-rend-to-class(.)), @sameAs, @sameAs, (), map {})
                        )
                    ...
                    case element(figDesc) return
                        (
                            html:inline($config, ., ("tei-figDesc", "<span", "class="icon">✏️</span>", css:map-rend-to-class(.)), .)
                        )
