...
module namespace model="http://www.tei-c.org/pm/models/liditest/print";
            ...
            return
                    ...
                    case element(licence) return
                        if ($parameters?mode='commentary') then
                            html:link($config, ., ("tei-licence1", css:map-rend-to-class(.)), ., @target, (), map {})
                        else
                    ...
                    case element(figure) return
                        html:figure($config, ., ("tei-figure", css:map-rend-to-class(.)), ., @figDesc)
                    ...
                    case element(author) return
                        if ($parameters?mode='commentary') then
                            ...
                            if (parent::bibl) then
                                html:inline($config, ., ("tei-author2", css:map-rend-to-class(.)), id (substring-after(@ref, '#'), doc("/db/apps/LiDi/data/bibl.xml")))
                            else
                                if (ancestor::teiHeader) then
                                    html:block($config, ., ("tei-author3", css:map-rend-to-class(.)), .)
                                else
                    ...
                    case element(ptr) return
                        html:link($config, ., ("tei-ptr1", css:map-rend-to-class(.)), "🔗", @target, (), map {})
                    ...
                    case element(pb) return
                        (
                            (: @facs :)
                            html:webcomponent($config, ., ("tei-pb1", css:map-rend-to-class(.)), ., 'pb-facs-link', map {"emit": 'transcription', "facs": let $d := substring-after(@facs, '#'), $f := id($d, root($parameters?root))/graphic[1]/@url return ``[`{$f}`/full/950,/0/default.jpg]``}),
                            html:break($config, ., css:get-rendition(., ("tei-pb2", css:map-rend-to-class(.))), ., 'page', if(@n) then concat(@n,' ') else '')
                        )

                    case element(epigraph) return
                        html:block($config, ., ("tei-epigraph", css:map-rend-to-class(.)), .)
                    case element(lb) return
                        html:break($config, ., css:get-rendition(., ("tei-lb1", css:map-rend-to-class(.))), ., 'line', @n)
                    ...
                    case element(publicationStmt) return
                        if ($parameters?mode='commentary') then
                            html:block($config, ., ("tei-publicationStmt1", css:map-rend-to-class(.)), .)
                        else
                    ...
                    case element(note) return
                        if (@place="margin-right" or @place="margin-left") then
                            printcss:note($config, ., ("tei-note", css:map-rend-to-class(.)), ., "margin", ())
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(ref) return
                        if ($parameters?mode='commentary') then
                            html:link($config, ., ("tei-ref1", css:map-rend-to-class(.)), @target, @target, (), map {})
                        else
                            if (@target) then
                    ...
                    case element(bibl) return
                        if (ancestor::body) then
                            printcss:alternate($config, ., ("tei-bibl", css:map-rend-to-class(.)), ., ., id (substring-after(@source, '#'), doc("/db/apps/LiDi/data/bibl.xml")))
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(title) return
                        if (parent::bibl) then
                            ...
                            if (parent::titleStmt/parent::fileDesc) then
                                ...
                                if ($parameters?header='short') then
                                    html:heading($config, ., ("tei-title6", css:map-rend-to-class(.)), ., 5)
                                else
                                            ...
                                            if (@level='s' or @level='j') then
                                                ...
                                                if (@level='u' or @level='a') then
                                                    (
                                                        ...
                                                        if (following-sibling::* and     (    ancestor::biblFull)) then
                                                            html:text($config, ., ("tei-title13", css:map-rend-to-class(.)), '. ')
                                                        else
                                                            ()
                                                    )
                                                    ...
                                                    (
                                                        html:inline($config, ., ("tei-title4", css:map-rend-to-class(.)), id (substring-after(@ref, '#'), doc("/db/apps/LiDi/data/bibl.xml"))),
                                                        html:link($config, ., ("tei-title5", css:map-rend-to-class(.)), ., @sameAs, (), map {})
                                                    )

                    case element(date) return
                        if ($parameters?mode='commentary') then
                            html:block($config, ., ("tei-date1", css:map-rend-to-class(.)), .)
                        else
                            if (@when) then
                                printcss:alternate($config, ., ("tei-date2", css:map-rend-to-class(.)), ., ., @when)
                            else
                                html:inline($config, ., ("tei-date3", css:map-rend-to-class(.)), .)
                    ...
                    case element(titleStmt) return
                        if ($parameters?mode='commentary') then
                            ...
                            if ($parameters?mode='title') then
                                html:heading($config, ., ("tei-titleStmt4", css:map-rend-to-class(.)), title[not(@type)], 5)
                            else
                                if ($parameters?header='short') then
                                    (
                                        html:link($config, ., ("tei-titleStmt5", css:map-rend-to-class(.)), title[1], $parameters?doc, (), map {}),
                                        html:block($config, ., ("tei-titleStmt6", css:map-rend-to-class(.)), subsequence(title, 2)),
                                        html:block($config, ., ("tei-titleStmt7", css:map-rend-to-class(.)), author)
                                    )

                                else
                                    html:block($config, ., ("tei-titleStmt8", css:map-rend-to-class(.)), .)
                        ...
                        if ($parameters?mode='commentary') then
                            ...
                            if ($parameters?header='short') then
                                (
                                    ...
                                    html:block($config, ., ("tei-fileDesc3", "header-short", css:map-rend-to-class(.)), /seriesStmt/title | (/seriesStmt/biblScope[position()=1]/@unit | /seriesStmt/biblScope[position()=1]/@n)),
                                    html:block($config, ., ("tei-fileDesc4", "header-short", css:map-rend-to-class(.)), //msIdentifier/repository),
                                    (: Output abstract :)
                                    html:block($config, ., ("tei-fileDesc5", "sample-description", css:map-rend-to-class(.)), //note/ab)
                                )
                        ...
                        if (@type="running-head") then
                            ...
                            if (@type="page-number") then
                                ...
                                if (ancestor::p or ancestor::ab) then
                                    html:inline($config, ., ("tei-fw3", css:map-rend-to-class(.)), .)
                                else
                                    html:block($config, ., ("tei-fw4", css:map-rend-to-class(.)), .)
                    case element(encodingDesc) return
                        if ($parameters?header='short') then
                            html:omit($config, ., ("tei-encodingDesc1", css:map-rend-to-class(.)), .)
                        else
                            if ($parameters?mode='commentary') then
                                let $params := 
                                    map {
                                ...
                                return
                                                                html:block(map:merge(($config, map:entry("template", true()))), ., ("tei-encodingDesc2", css:map-rend-to-class(.)), $content)
                            else
                                html:inline($config, ., ("tei-encodingDesc3", css:map-rend-to-class(.)), .)
                    case element(quote) return
                        printcss:alternate($config, ., ("tei-quote", css:map-rend-to-class(.)), ., ., id (substring-after(@source, '#'), doc("/db/apps/LiDi/data/bibl.xml")))
                    ...
                    case element(seg) return
                        if (@xml:lang and @source) then
                            ...
                            return
                                                        html:figure(map:merge(($config, map:entry("template", true()))), ., ("tei-seg1", css:map-rend-to-class(.)), $content, ())
                        else
                            if (@xml:lang) then
                                html:figure($config, ., ("tei-seg2", css:map-rend-to-class(.)), ., ())
                            else
                                if (@source) then
                                    html:link($config, ., ("tei-seg3", css:map-rend-to-class(.)), ., @source, "_blank", map {})
                                else
                                    html:inline($config, ., ("tei-seg4", css:map-rend-to-class(.)), .)
                    ...
                    case element(cb) return
                        html:omit($config, ., ("tei-cb", css:map-rend-to-class(.)), .)
                    case element(persName) return
                        if (ancestor::listPerson) then
                            html:block($config, ., ("tei-persName1", css:map-rend-to-class(.)), .)
                        else
                            if (ancestor::body) then
                                printcss:alternate($config, ., ("tei-persName2", css:map-rend-to-class(.)), ., ., id (substring-after(@ref, '#'), doc("/db/apps/LiDi/data/people.xml")))
                            else
                                $config?apply($config, ./node())
                    case element(placeName) return
                        if (ancestor::body) then
                            printcss:alternate($config, ., ("tei-placeName", css:map-rend-to-class(.)), ., ., id (substring-after(@ref, '#'), doc("/db/apps/LiDi/data/place.xml")))
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(person) return
                        if (@sameAs) then
                            html:link($config, ., ("tei-person1", css:map-rend-to-class(.)), persName, @sameAs, "_blank", map {})
                        else
                            html:inline($config, ., ("tei-person2", css:map-rend-to-class(.)), persName)
                    case element(place) return
                        if (@sameAs) then
                            html:link($config, ., ("tei-place1", css:map-rend-to-class(.)), ., @sameAs, "_blank", map {})
                        else
                            html:inline($config, ., ("tei-place2", css:map-rend-to-class(.)), placeName)
                    ...
                    case element(citedRange) return
                        html:inline($config, ., ("tei-citedRange", css:map-rend-to-class(.)), .)
                    case element(sponsor) return
                        html:omit($config, ., ("tei-sponsor", css:map-rend-to-class(.)), .)
                    case element(editionStmt) return
                        if ($parameters?mode='commentary') then
                            html:block($config, ., ("tei-editionStmt", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    case element(respStmt) return
                        html:omit($config, ., ("tei-respStmt", css:map-rend-to-class(.)), .)
                    case element(principal) return
                        if ($parameters?mode='commentary') then
                            html:block($config, ., ("tei-principal", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    case element(funder) return
                        html:omit($config, ., ("tei-funder", css:map-rend-to-class(.)), .)
                    case element(sourceDesc) return
                        if ($parameters?mode='commentary') then
                            html:block($config, ., ("tei-sourceDesc", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    case element(msDesc) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        html:block(map:merge(($config, map:entry("template", true()))), ., ("tei-msDesc1", css:map-rend-to-class(.)), $content)
                        else
                            html:inline($config, ., ("tei-msDesc2", css:map-rend-to-class(.)), .)
                    case element(physDesc) return
                        if ($parameters?mode='commentary') then
                            html:block($config, ., ("tei-physDesc", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    case element(seriesStmt) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        html:block(map:merge(($config, map:entry("template", true()))), ., ("tei-seriesStmt", css:map-rend-to-class(.)), $content)
                        else
                            $config?apply($config, ./node())
                    case element(editorialDecl) return
                        if ($parameters?mode='commentary') then
                            html:block($config, ., ("tei-editorialDecl1", css:map-rend-to-class(.)), .)
                        else
                            if ($parameters?header='short') then
                                html:omit($config, ., ("tei-editorialDecl2", css:map-rend-to-class(.)), .)
                            else
                                $config?apply($config, ./node())
                    case element(availability) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        html:block(map:merge(($config, map:entry("template", true()))), ., ("tei-availability", css:map-rend-to-class(.)), $content)
                        else
                            $config?apply($config, ./node())
                    case element(figDesc) return
                        printcss:alternate($config, ., ("tei-figDesc", css:map-rend-to-class(.)), ., "🖼️", .)
                    case element(term) return
                        ...
                        return
                                                html:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-term", css:map-rend-to-class(.)), $content)
                    case element(classDecl) return
                        if ($parameters?mode='commentary') then
                            ...
                            return
                                                        html:block(map:merge(($config, map:entry("template", true()))), ., ("tei-classDecl", css:map-rend-to-class(.)), $content)
                        else
                            $config?apply($config, ./node())
